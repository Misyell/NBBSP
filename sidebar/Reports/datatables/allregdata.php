		...
		<table id="example" class="display nowrap" style="width:100%">
			<thead>
				<tr>
					...
					<?php
                        $sql = "SELECT myFile, housenum, residenttype, housetype, contact, dob, lname, fname FROM tblresident";
                        $result= mysqli_query($conn, $sql);
                        $row = mysqli_fetch_array($result);

                    if ($result-> num_rows > 0){
                        while ($row = $result->fetch_assoc()){
                            echo "<tr><td>" . $row["myFile"]. "</td><td>". $row["housenum"]. "</td><td>". $row["residenttype"]. "</td><td>" . $row["housetype"]. "</td><td>". $row["contact"] ."</td><td>". $row["dob"]. "</td><td>" . $row["lname"]. "</td><td>" . $row["fname"]."</td></tr>";
                        }
                    } else{
                        echo "No data";
                    }
